...
test_a = pd.read_csv('D:\kaggle\\alimm\\round1_ijcai_18_test_b_20180418.txt', sep=" ")
...
gbm_sub = lgb.LGBMClassifier(max_depth=2)
gbm_sub.fit(X_train, y_train)

sub = pd.DataFrame()

sub['instance_id'] = list(test_index)
sub['predicted_score'] = gbm_sub.predict_proba(X_test)[:, 1]

sub.to_csv('D:\kaggle\\alimm\\baseline_10.csv', sep=" ", index=False)

# 0.08276 线上结果
# 0.0826226340838 线下结果
